...
#             replace=True)
    ...
    def sample_initial_galaxy(self):
        ...
        # FIX: cogsworth.sfh.concat does not concatenate v_x and v_y, so they have the wrong shape
        # this causes the sort below to fail on the slicing step
        # since we don't need them (we recalculate velocities below), we can just remove them
        if hasattr(self._initial_galaxy, "v_x"):
            del self._initial_galaxy.v_x
        if hasattr(self._initial_galaxy, "v_y"):
            del self._initial_galaxy.v_y
